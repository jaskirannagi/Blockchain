...
def transactionbuffer(): #call every loop, will reset tranasaction buffer when enough transactions can generate a block
    ...
    while len(txnBuffer) > 0: # for the length of the buffer
        ...
        while (len(txnBuffer) > 0) & (len(txnList) < blockSizeLimit): # while there is transactions and the transaction list (array to be added to the block) is less than 5
            ...
            if validTxn:           # If we got a valid state, not 'False'
                ...
                state = updateState(newTxn,state) #update the state of the the ledger
            else:
        ...
        ## Make a block
        myBlock = makeBlock(txnList,chain) #create the bock from the details
        chain.append(myBlock) #add the block to the chain
        


def updateState(txn, state): # state (number of transactions, users, etc)
...
while(1):
    ...
    if(setup==0):  #run setup (only once)
        geninissbloc() #set up starting bloc
        setup=1 #dont run again
